...
| **2:00 – 2:15** | 15s | Cut to OC — back at the desk, a touch more relaxed now. | OC: *"Existing tools fire static prompts at a target. Mesmer runs an adaptive ReAct agent that learns from every attempt — a persistent attack graph, a tiered attack ladder, modules grounded in published cognitive-science and red-team research."* | B-roll inserts (1s each): `modules/techniques/` directory tree, a slow scroll of `core/agent/engine.py`, the TAPER tier diagram from the README. | **Depth (20%) + gap framing.** "Existing tools fire static prompts" lets the judge silently fill in *Garak/Promptfoo* themselves — more persuasive than naming competitors. The pivot to "adaptive ReAct agent that learns" is the actual differentiation. |
