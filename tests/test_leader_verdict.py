...
* Status is execution lifecycle only; objective result is carried by score,
  reflection, and module output.
* ``AttackNode.is_leader_verdict`` correctly distinguishes it by source.
* Attempt-centric views (``get_explored_nodes`` and
  ``conversation_history`` when filtered) behave sanely in the presence of a
  leader-verdict node.
...
class TestLeaderVerdictNode:
    def test_node_attaches_to_latest_submodule(self):
        ...
        assert leader.status == NodeStatus.COMPLETED.value
